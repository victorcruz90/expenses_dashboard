...
def render(app: Dash, data: pd) -> html.Div:
    ...
    def update_graph(rows, fortnight_values: list):
        ...
        if fortnight_values:
            ...
            fig.update_yaxes(showgrid=False, range=[0,df_income.max(numeric_only=True)], tickfont=dict(family='Arial', color='black', size=14),showline=True, linewidth=2, linecolor='black', mirror=False)
